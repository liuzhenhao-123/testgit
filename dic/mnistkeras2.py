...
# # 设置图像的维度顺序（‘tf’或‘th’）# 当前的维度顺序如果为'th'，则输入图片数据时的顺序为：channels,rows,cols，否则:rows,cols,channels
# K.set_image_data_format('channels_first')

# 设置随机数种子，以保证每次训练时顺序一样。
seed = 7
...
print(X_test.shape)  # (10000, 1, 28, 28)
print(y_train.shape)  # (60000, 10)
print(y_val.shape)  # (60000, 10)
print(y_test.shape)  # 10

# 接下来构造CNN。

# 第一层是卷积层。该层有32个feature map,或者叫滤波器，作为模型的输入层，接受[pixels][width][height]大小的输入数据。
# feature map的大小是5*5，其输出接一个‘relu’激活函数。
